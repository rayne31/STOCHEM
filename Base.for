...
C
     ...
     &+ Y(JB,55) +Y(JB,231)  +Y(JB,239) + Y(JB,240)
C
...
C
        DO 1010 I=1,NC
          FLUX(JB,900+I)=FLUX(JB,900+I)+EM(JB,I)*DTS/M(JB)
          FLUX(JB,1200+I)=FLUX(JB,1200+I)+Y(JB,I)*DD(JB,I)*DTS/M(JB)
          FLUX(JB,1500+I)=FLUX(JB,1500+I)+Y(JB,I)*DW(JB,I)*DTS/M(JB)
 1010   CONTINUE
 1020 CONTINUE
C
C      debug concentrations for one cell.
...
C                                  Anth  ,Biomass,Veg   ,Soil  ,Oceans
      ...
      DATA (CLASS(IC,42),IC=1,5)  /2.0,    3.69,  20.7,  0.0,  34.0 / ! CH3CHO
      DATA (CLASS(IC,28),IC=1,5)  /9.50,  0.74,   0.06,  0.0,  8.24 / ! NC4H10
      DATA (CLASS(IC,16),IC=1,5)  /84.50,   3.14,   0.0,  0.0,  0.0 / ! SO2
      DATA (CLASS(IC,30),IC=1,5)  /6.56,   5.04,  26.9, 0.26,  1.4 / ! C2H4
      DATA (CLASS(IC,32),IC=1,5)  /2.77,   2.90,  15.8, 0.16,  1.52 / ! C3H6
      DATA (CLASS(IC,67),IC=1,5)  /6.43,    0.37,   0.0,  0.0,  0.0 / ! OXYL
      DATA (CLASS(IC,25),IC=1,5)  /5.94,   0.98,  0.03, 0.0,  1.29/ ! C3H8
      DATA (CLASS(IC,76),IC=1,5) /7.75,  9.09,   99.6, 0.0,  24.0 / ! CH3OH
      DATA (CLASS(IC,73),IC=1,5)  /0.78,  1.90,  43.7, 0.0,  0.0 / ! CH3COCH3
      DATA (CLASS(IC,43),IC=1,5)  /0.13,    0.50,   535.0, 0.0, 1.90 / ! C5H8
      DATA (CLASS(IC,64),IC=1,5)  /5.71,    1.07,   1.24,  0.1,  0.0 / ! TOLUENE
      DATA (CLASS(IC,47),IC=1,5)  /0.16,    1.0,  108.2, 0.0, 0.0 / ! APINENE
      DATA (CLASS(IC,53),IC=1,5)  /0.08,    0.5,   54.1, 0.0, 0.0 / ! BPINENE
      DATA (CLASS(IC,61),IC=1,5)  /4.13,    2.3,   0.0,   0.07,  0.0 / ! BENZENE      
      DATA (CLASS(IC,40),IC=1,5)  /2.23,   1.46,  3.70,   1.76,  0.0 / ! HCOOH
      DATA (CLASS(IC,41),IC=1,5)  /9.67,  11.23,  3.7,   3.42,  0.0 / ! CH3CO2H
      DATA (CLASS(IC,77),IC=1,5)  /6.00,  0.18,  20.7,   0.0,  0.0 / ! C2H5OH
      DATA (CLASS(IC,101),IC=1,5) /1.20,  1.21  ,0.0   ,0.0   ,0.0 / ! MEK
      DATA (CLASS(IC,71),IC=1,5)  /3.13,   0.0   ,0.0   ,0.0   ,0.0 / ! C2H5CHO
      DATA (CLASS(IC,59),IC=1,5)  /2.80,   1.16,  0.0,   0.13,   0.0 / ! C2H2
      DATA (CLASS(IC,34),IC=1,5)  /5.21,  1.11  ,8.0   ,0.0   ,0.0 / ! TBUT2ENE
      DATA (CLASS(IC,220),IC=1,5)  /0.0,  0.01,  0.0,  0.0,  38.6/ ! DMS
      DATA (CLASS(IC,227),IC=1,5)  /0.09,  0.03,  0.06,  0.0,  0.01/ ! CH3BR
      DATA (CLASS(IC,228),IC=1,5)  /53.6,  6.08,  0.0,  36.17,  8.2/ ! NH3
      DATA (CLASS(IC,218),IC=1,5)  /11.10,  23.80,  0.0,  0.0,  0.0/ ! EMPOA
